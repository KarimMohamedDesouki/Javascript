var firstName =prompt("what is your firstname ?");
var lastName=prompt("what is your Lastname ?");
var birthyear=prompt("enter your age ?");
var fullName=console.log(" Welcome ", firstName + " " + lastName + " you are " + birthyear , "years old");


console.log("it’s the first release of a calculator that only has a summation feature.")
var firstNumber=Number(prompt("enter the first number" , firstNumber));
var secondNumber=Number(prompt("enter the second number" , secondNumber)); 
console.log("the sum of the two numbers is:", firstNumber + secondNumber);