...
// 1. Write a JavaScript program to compare two objects to determine if the first 
// contains equivalent property values to the second one.

// ***hasOwnProperty() value parameter return boolean checks if it has the property value
...
function compareObjects(obj1, obj2) {
    ...
    if (keys1.length !== keys2.length) {// Check if the length of the values are different
        return false;
    }

   
    for (let key of keys1) { // Iterate through keys of the first object
        
        if (!obj2.hasOwnProperty(key)) {// Check if the key exists in the second object
            return false;
        }

        if (obj1[key] !== obj2[key]) {//check if the values of the object1 is equal to object 2
            return false;
        }
    }

     //If checks passed, object 1 contains equvilant values in object 2
    return true;
}
...
// ***select() no parameter no return it selects the data for you
// ***execCommand() many parameters it copies the data for you
// dupericated
...
// ***Split(string,delimeter)  //return array 
...
function csvToArray(csvString) {
        ...
        for (let i = 0; i < rows.length; i++) { // Iterate over rows
            ...
            result.push(columns); //Add the columns to the result array
        }
...
// ***trim() no parameters return string remove the spacing 
// ***shift() no parameters return removed element from array takes the first element from the array


function csvToArrayOfObjects(csvString) {
    
const rows = csvString.trim().split('\n'); //Split the CSV string into rows  
const titles = rows.shift().split(','); //Extract the title row and split it into titles
        console.log(titles);
...
const result = [];
    ...
    rows.forEach(row => {
        ...
        const obj = {}; //Create an object for the current row

        titles.forEach((title, index) => { //Map columns to object properties using titles
            obj[title.trim()] = columns[index].trim();
        });
...
const arrayOfObjects = csvToArrayOfObjects(csvString);
...
// ***filter callback function input element-The current element being processed in the array.  index-The index of the current element being processed in the array.   array-The array filter() was called upon.  return copy of the array 
// ***include parameter value to search with return boolean 
// ***join parameter value to join with like (,) return string of the array 

function objectsToCsvString(data, columns) {
    if (data.length === 0) {// Ensure data is not empty
    return "empty string";
    }

    const headers = Object.keys(data[0]);    // Extract keys from the first object to use as headers     
    const filteredHeaders = headers.filter(header => columns.includes(header));  //to get the titles only        
    let csvString = filteredHeaders.join(',') + '\n';//convert the array of titles to be a string
    console.log(csvString);
           
    data.forEach(obj => { // Iterate through values  object in the array
        const values = filteredHeaders.map(header => obj[header]); //Extract values for the specified columns
...
function findValueByKey(obj, key) {
    const stack = [obj]; // Initialize stack with the root object

    while (stack.length > 0) {
        const currentObj = stack.pop(); // Pop the last object from the stack

        
        if (currentObj && typeof currentObj === 'object') { //Check if the current object is not null or undefined
            
            if (currentObj.hasOwnProperty(key)) {// Check if the current object has the key
                return currentObj[key];  //Return the value if key is found
            }

             //If the current object is not the target, iterate over its properties
            for (let prop in currentObj) {
                if (currentObj.hasOwnProperty(prop)) {
                    stack.push(currentObj[prop]); // Push nested objects to the stack
                }
            }
        }
    }
...
const value1 = findValueByKey(nestedObject, keyToFind1);
const value2 = findValueByKey(nestedObject, keyToFind2);

console.log(`Value for key "${keyToFind1}":`, value1); // Value for key "c": 123
console.log(`Value for key "${keyToFind2}":`, value2); // Value for key "f": 456
...
function numbertoarray(numbers){
            var i;
            const numstring = numbers.toString();//convert the array to string
            console.log(`the array entered: ${numstring}`);
            const splitted = numstring.split('');//arry but it is in string
            console.log(`the array as string ${splitted}`);
            
            const result = [];
            for(i=0;i<splitted.length;i++){// for loop to parseint all the array and to put it in new array
                const strtoint = parseInt(splitted[i]);
                console.log(strtoint);
                result.push(strtoint);

            }
            return (`the array as numbers ${result}`);
        }

        const numbers1 = 123411234;
        console.log(numbertoarray(numbers1));
...
function search(arr, number) {
    //filterfunction to filter the values that we donot need from the array
    const result = arr.filter(item => !number.includes(item));
    console.log(result);
    return result;
}
//the output of the value should be the same of the variable which will be holding the value
const arr = [1, 2, 3, 4, 5, 6, 7, 8];
const searchvalues = [2,6,7];
const newArray = search(arr, searchvalues);
...
function extract(arr,indexes) {
        const result = [];
        let i;
        for(i=0;i<indexes.length;i++)// for loop on the array 
        {
            const index = indexes[i];
            if(index >= 0 && index < arr.length){//see the boundries if greater then undefiend
